...
module soc_interconnect #(
    ...
    `ifndef NO_TO_CLUSTER_AXI
        localparam N_PERIPHS    = 3+N_L2_BANKS_PRI         ;
    `else
        localparam N_PERIPHS    = 2+N_L2_BANKS_PRI         ;
    `endif
    localparam L2_OFFSET_PRI = 32'h0000_4000; // FIXME Put the right FORMULA
    ...
    `else
        localparam logic [N_PERIPHS-1:0][ADDR_WIDTH-1:0] PER_START_ADDR = { 32'h1C00_8000, 32'h1C00_0000, 32'h1A00_0000,  32'h1A10_0000};
        localparam logic [N_PERIPHS-1:0][ADDR_WIDTH-1:0] PER_END_ADDR   = { 32'h1C01_0000, 32'h1C00_8000, 32'h1A04_0000,  32'h1A20_0000};
    `endif
    ...
    `else
    // ROM BINDING
        assign rom_csn_o                     = ~PER_data_req_TO_BRIDGE[1];
        assign rom_add_o                     = PER_data_add_TO_BRIDGE[1];
        assign PER_data_r_rdata_TO_BRIDGE[1] = rom_rdata_i;
    
        assign PER_data_gnt_TO_BRIDGE[1] = 1'b1;
        always_ff @(posedge clk or negedge rst_n)
        begin : proc_
            if(~rst_n) begin
                PER_data_r_valid_TO_BRIDGE[1] <= '0;
            end
            else begin
                PER_data_r_ID_TO_BRIDGE[1]    <= PER_data_ID_TO_BRIDGE[1];
                PER_data_r_valid_TO_BRIDGE[1] <= PER_data_req_TO_BRIDGE[1];
                PER_data_r_aux_TO_BRIDGE[1]   <= PER_data_aux_TO_BRIDGE[1];
                PER_data_r_opc_TO_BRIDGE[1]   <= 1'b0;
            end
        end
    `endif
    ...
    generate
        // Private mem Binding
        for (k = 0; k< N_L2_BANKS_PRI-1; k++) begin
         `ifndef NO_TO_CLUSTER_AXI
        ...
        `else
            assign L2_pri_D_o   [k] =  PER_data_wdata_TO_BRIDGE[k+2];
            assign L2_pri_A_o   [k] =  PER_data_add_TO_BRIDGE  [k+2][ADDR_L2_PRI_WIDTH+1:2];
            assign L2_pri_CEN_o [k] = ~PER_data_req_TO_BRIDGE  [k+2];
            assign L2_pri_WEN_o [k] =  PER_data_wen_TO_BRIDGE  [k+2];
            assign L2_pri_BE_o  [k] =  PER_data_be_TO_BRIDGE   [k+2];
            
            assign PER_data_r_rdata_TO_BRIDGE[k+2] = L2_pri_Q_i  [k];
            assign PER_data_gnt_TO_BRIDGE[k+2]     = 1'b1;
            assign PER_data_r_opc_TO_BRIDGE[k+2]   = '0;
            
            always_ff @(posedge clk or negedge rst_n)
            begin : proc_L2_CH_pri_rvalid_gen
                if(~rst_n) begin
                    PER_data_r_valid_TO_BRIDGE[k+2] <= '0;
                    PER_data_r_ID_TO_BRIDGE[k+2]    <= '0;
                    PER_data_r_aux_TO_BRIDGE[k+2]   <= '0;
                end
                else begin
                    PER_data_r_valid_TO_BRIDGE[k+2] <= PER_data_req_TO_BRIDGE[k+2];
                    PER_data_r_ID_TO_BRIDGE[k+2]    <= PER_data_ID_TO_BRIDGE[k+2];
                    PER_data_r_aux_TO_BRIDGE[k+2]   <= PER_data_aux_TO_BRIDGE[k+2];
                end
            end
        `endif        
        end
    endgenerate
/*l2 pri1 binding*/
            assign L2_pri_D_o   [1] =  PER_data_wdata_TO_BRIDGE[1+2];
            assign L2_pri_A_o   [1] =  PER_data_add_TO_BRIDGE  [1+2][ADDR_L2_PRI_WIDTH+1:2];
            assign L2_pri_CEN_o [1] = ~PER_data_req_TO_BRIDGE  [1+2];
            assign L2_pri_WEN_o [1] =  PER_data_wen_TO_BRIDGE  [1+2];
            assign L2_pri_BE_o  [1] =  PER_data_be_TO_BRIDGE   [1+2];
            
            assign PER_data_r_rdata_TO_BRIDGE[1+2] = L2_pri_Q_i  [1];
            //assign PER_data_gnt_TO_BRIDGE[1+2]     = 1'b1;
            assign PER_data_gnt_TO_BRIDGE[1+2]     = l2_gnt_i;
            assign PER_data_r_opc_TO_BRIDGE[1+2]   = '0;
            
            always_ff @(posedge clk or negedge rst_n)
            begin : proc_L2_CH_pri_rvalid_gen
                if(~rst_n) begin
                    PER_data_r_valid_TO_BRIDGE[1+2] <= '0;
                    PER_data_r_ID_TO_BRIDGE[1+2]    <= '0;
                    PER_data_r_aux_TO_BRIDGE[1+2]   <= '0;
                end
                else begin
                    PER_data_r_valid_TO_BRIDGE[1+2] <= l2_rvalid_i;
                    PER_data_r_ID_TO_BRIDGE[1+2]    <= PER_data_ID_TO_BRIDGE[1+2];
                    PER_data_r_aux_TO_BRIDGE[1+2]   <= PER_data_aux_TO_BRIDGE[1+2];
                end
            end
/*l2 pri1 binding*/            
    always_comb
