...
Parameter UDMA_EVENTS bound to: 29 - type: integer 
Parameter SOC_EVENTS bound to: 3 - type: integer 
*/
...
module soc_peripherals /* #(
    ...
    output logic [8-1:0]      cl_event_data_o,
    output logic                       cl_event_valid_o,
    input  logic                       cl_event_ready_i,
    output logic [8-1:0]      fc_event_data_o,
...
localparam EVNT_WIDTH=8;
    ...
    reg [3:0] LED=4'b0000;
    assign led_o=LED;
    assign s_led_bus.pready=1'b1;
    always @(posedge clk_i, negedge rst_ni)
    begin
      if(rst_ni==1'b0)
        LED<=4'b0000;
      else
        if(s_led_bus.paddr==32'h1A12_0000 && s_led_bus.pwrite && s_led_bus.psel && s_led_bus.penable)
        begin
          LED <= s_led_bus.pwdata[3:0];
        end
        else
          LED <= LED;
    end
